...
public class MessageFormat01 {
    ...
    List<Rec> recList = List.of(
            new Rec(
                    "基本数值格式化",
                    "{0}",
                    new Object[] {1234567890.123456},
                    Map.of(CHINESE, "1,234,567,890.123", ENGLISH, "1,234,567,890.123")),
            new Rec("百分比格式", "{0,number,percent}", new Object[] {1.23456}, Map.of(CHINESE, "123%", ENGLISH, "123%")),
            new Rec(
                    "货币格式",
                    "{0,number,currency}",
                    new Object[] {1234567890.123},
            ...
            new Rec(
                    ...
                    "{},{0},{1},{2}",
                    new Object[] {"aaa", "bbb", "ccc"},
                    Map.of(CHINESE, "IllegalArgumentException", ENGLISH, "IllegalArgumentException"),
                    true));
        ...
        for (int i = 0; i < recList.size(); i++) {
            ...
            try {
                if (rec.shouldThrowException) {
                    ...
                    assertEquals(
                            String.format("Record [%d] %s failed", i, rec.description), expectedResult, actualResult);
                    log.info("[{}] ✓ {} - Pattern: {} - Result: {}", i, rec.description, rec.pattern, actualResult);
                }
            } catch (AssertionError e) {
                String actualResult = MessageFormat.format(rec.pattern, rec.args);
                throw new AssertionError(
                        String.format(
                                "Record [%d] %s failed. Expected: %s, Actual: %s",
                                i, rec.description, expectedResult, actualResult),
                        e);
            } catch (Exception e) {
                throw new RuntimeException(
                        String.format("Record [%d] %s threw unexpected exception", i, rec.description), e);
            }
        }
